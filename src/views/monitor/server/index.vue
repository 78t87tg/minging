<template>
    ...
    <MenuComponent @update-current-video="setCurrentVideo(arguments)" :videoList="videoList" class="VideoMenu"  v-if="VideoMode"></MenuComponent>
      <div class="video-grid" v-if="VideoMode">
        <div v-for="video in paginatedVideoList(videoListID)" :key="video.id" class="video-item" :class="{ selected: currentVideo === video.id }">
          <h3 class="title1">{{ video.title }}</h3>
          <a-button type="primary" @click="() => changeView(video.id)" size="small" class="videoItemButton">
            多角度
          </a-button>
      ...
      <div class="video-grid1"  v-else>
        <div v-for="video in videoListThree" :key="video.title" class="video-item-three" >
          <h3 class="title1">{{ video.title }}</h3>
          <a-button type="primary" @click="() => changeView(video.id)" size="small" class="videoItemButton">
            返回
          </a-button>
...
methods: {
  ...
  .video-component {
    ...
    width: 100%;
    height: 100%;
    background-image: url("./mbgc.jpg"); /* 设置背景图片的URL */
